...
bool ShaderProgram::add_shader_from_file(const std::string& path, GLuint shaderType)
{
...
bool ShaderProgram::add_shader_from_source(const std::string& shaderSource, GLuint shaderType)
{
    auto newShader = std::make_shared<Shader>();
...
bool ShaderProgram::add_shader(Shader* shader)
{
...
{
    if (linked_) glUseProgram(programID_);
}
...
{
    int location = glGetUniformLocation(programID_, name.c_str());
    glUniform2fv(location, count, reinterpret_cast<GLfloat*>(vectors)); // cast vectors to GLfloat arrays
}
...
{
    int location = glGetUniformLocation(programID_, name.c_str());
    glUniform3fv(location, count, reinterpret_cast<GLfloat*>(vectors)); // cast vectors to GLfloat arrays
}
...
{
    int location = glGetUniformLocation(programID_, name.c_str());
    glUniform4fv(location, count, reinterpret_cast<GLfloat*>(vectors)); // cast vectors to GLfloat arrays
}
...
{
    int location = glGetUniformLocation(programID_, name.c_str());
    glUniformMatrix4fv(location, count, GL_FALSE, reinterpret_cast<GLfloat*>(matrices));
}
